...
Following the initial round of Black Box testing, a formal peer technical audit was conducted on the architecture. This surfaced critical vulnerabilities in the original prototype, primarily the storage of raw passwords in the database and the hard-coding of the `SECRET_KEY` in settings. To elevate the application to production-ready status, a rigorous remediation sprint was executed:
...
First, the string-based skill matching algorithm lacks semantic comprehension. A student entering "ReactJS" will not match a prerequisite of "React.js" because the regex parser relies on exact string intersection rather than semantic equivalence. Future iterations would require Natural Language Processing integration to map synonyms and handle common abbreviation variants, significantly improving matching accuracy across a real student cohort where terminology is inconsistent.
Second, the SQLite backend poses scalability boundaries. SQLite handles concurrent write requests poorly — it uses file-level locking, meaning simultaneous writes from multiple users block one another. While sufficient for a departmental cohort of approximately 100 students, an institution-wide deployment of thousands of students submitting preferences simultaneously on a deadline day would inevitably cause database locking overhead and degraded performance. A production rollout would require migration to a server-based relational database such as PostgreSQL or MySQL to handle concurrent load safely.
Third, the Fairness Engine's architecture currently violates the Single Responsibility Principle. The orchestration logic is bundled within the Django views and a centralised utils.py file, tightly coupling the mathematical scoring engine to the web framework. While functional at departmental scale, this design complicates unit isolation and makes the engine difficult to test independently of the Django environment. A more professional approach for future versions would abstract this into a dedicated Service Layer, separating the database ORM interactions from the scoring algorithm and enabling independent testing of the allocation logic.
Finally, the Fairness Engine currently lacks a fallback pathway for students who match zero preferences across all three of their choices. Such students are deferred entirely to manual allocation, which partially reintroduces the administrative burden the system was designed to eliminate. A future version could implement an automatic secondary matching round using a broader skill-overlap threshold for unallocated students before escalating to manual review.

7.4 Future Work
While mechanically robust, ProjectMatch has significant avenues for future expansion that would extend its applicability beyond a single department.
A primary addition would be Single Sign-On (SSO) integration with university active directories, eliminating local password storage and enabling seamless authentication through existing institutional credentials.
The Fairness Engine could be augmented with a machine learning recommendation layer, actively suggesting ideal projects to students based on natural language processing of recorded research interests rather than relying solely on exact prerequisite intersection. This would address the current string-matching limitation and produce meaningfully more accurate suitability scores across a diverse student cohort.
A particularly valuable institutional extension would be the introduction of Departmental Allocation Profiles. Rather than a single university-wide weighting configuration, an administrator could select a department — such as Engineering and Physical Sciences (EPS), Business, or Law — and apply a discipline-specific weighting preset before triggering the Fairness Engine. EPS projects, for example, often involve laboratory safety requirements or highly technical skill thresholds, making a higher qualification weighting appropriate. In contrast, Law or Business projects may prioritise student interest and personal motivation as stronger predictors of project success, favouring a higher preference weighting. This filtering mechanism would allow ProjectMatch to scale to an institution-wide deployment while respecting the distinct allocation priorities of each academic school.
Expanding the notification engine to include external email dispatches would further reduce communication bottlenecks, and migrating from SQLite to a production-grade database would support the concurrent load of a full institutional rollout.
